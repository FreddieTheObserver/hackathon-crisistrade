...
- **Admin moderation:** `User.role` (`"user"` | `"admin"`) carried in the session JWT and returned by `/auth/me`. `isAdmin`/`requireAdmin` live in `require-auth.ts`. Admins **bypass** every board's owner-check (`isAdmin || owner`) at the API layer, so any board's existing endpoints already accept admin moderation (suspend/ban/restore/delete) — no separate admin API. Admins are created via seed only; signup always creates a plain `user` (no self-elevation). Status enums gained `suspended`/`banned` per board for moderation.
  - **Current frontend coverage:** the admin dashboard UI (`frontend/src/modules/exchange/admin/`) surfaces both Safe Exchange Points management (`/admin` = "Locations Management") and a **cross-board moderation view** (`/admin/overview`) that lists Trades, Donations, and Emergency Requests with suspend/ban/restore/delete actions, all driven through each board's existing endpoints via the admin owner-check bypass.
- **Frontend session state:** a global `AuthProvider` (`frontend/src/auth/AuthContext.tsx`, Area I) resolves the session via `GET /auth/me` on load and holds the current user; `useAuth()` exposes `user`/`loading`/`setUser`/`refresh`/`logout`. `/login` and `/signup` are separate routes whose forms call `/auth/login` and `/auth/signup` and push the user into context; the navbar shows the user and a logout action. `ProtectedRoute` (`frontend/src/middlewares/ProtectedRoute.tsx`) is now real — it gates the `/` app shell and redirects unauthenticated visitors to `/login`. `RequireAdmin` (`frontend/src/middlewares/RequireAdmin.tsx`) layers on top to gate the `/admin*` routes to `role === "admin"` (non-admins are redirected to the app home); admins reach the dashboard via an **Admin** link the navbar shows only to admin sessions.
- **Still deferred to frontend integration:** the Rules & Regulations acceptance gate. (User-account moderation — suspending or banning whole accounts rather than individual posts — is also not implemented; moderation operates per-post only.)
