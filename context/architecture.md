...
- **Admin moderation:** `User.role` (`"user"` | `"admin"`) carried in the session JWT and returned by `/auth/me`. `isAdmin`/`requireAdmin` live in `require-auth.ts`. Admins **bypass** every board's owner-check (`isAdmin || owner`) at the API layer, so any board's existing endpoints already accept admin moderation (suspend/ban/restore/delete) — no separate admin API. Admins are created via seed only; signup always creates a plain `user` (no self-elevation). Status enums gained `suspended`/`banned` per board for moderation.
  - **Current frontend coverage:** the admin dashboard UI (`frontend/src/modules/exchange/admin/`) currently surfaces **Safe Exchange Points only** (`/admin` = "Locations Management"). The backend bypass makes the other three boards moderatable through the same pattern, but a cross-board admin UI for Trades, Donations, and Emergency Requests is **not yet built** and remains future work.
...
- **Still deferred to frontend integration:** the Rules & Regulations acceptance gate, and a cross-board admin moderation UI (only Safe Exchange Points has an admin surface today — see Admin moderation above).
